...
describe("Container <CommentBox />", () => {
    ...
    const connectedProps = {
        ...
        onToggleReplyList: function toggleReplyList() {}
    }
    ...
    it("should add user info to reply list", () => {
        const result = AddUserInfoToReply(state.resources.replies, state.resources.users)
        expect(result[0].user).toEqual(state.resources.users[0])
        expect(result[0].toUser).toBe(undefined)
        expect(result[1].toUser).toEqual(state.resources.users[0])
        expect(result[1].user).toEqual(state.resources.users[1])
    })
